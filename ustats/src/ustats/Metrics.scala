...
import types.ConcurrentLinkedDeque
...
/** A memory-efficient, concurrent-friendly metrics collection interface.
  ...
  */
class Metrics(BlockSize: Int = 128) extends geny.Writable {
  ...
  override def httpContentType: Option[String] = Some("text/plain; charset=utf-8")
  override def writeBytesTo(out: OutputStream): Unit = writeMetricsTo(out, includeInfo = true)
  ...
  class counters(
    ...
    transparent inline final def labelled(
      inline labels: String*
    ): MetricsGroup[Counter] =
      addInfo(name, help, "counter")
      MetricsGroup.refine(
        labels,
